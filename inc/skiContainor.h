 ...
 * para2: capacity of list, use SKILIST_NOLIMIT to make it max
 ...
 * return: size of list after inserting on success, 0 if error
 */
 ...
 * return: size of list before deleting on success, 0 if error
 */
 ...
 * para2: function to deal every node in list
   		for each would stop if func2 return 0
   		after all nodes dealed, func2 would be called with NULL in 1st para
...
/**
 * breif: resize the capacity of list
 * Para1: handler of list to be operated
 * para2: new capacity, it wouldn't be set when less then size of list
 * return: void
 */
 ...
 * para2: function to deal every node
   		for each would stop when func3 return 0
